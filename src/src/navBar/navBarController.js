...
const stat = `LET r = (FOR d IN _statistics sort d.time desc limit @time RETURN d)
...
angularModule.push((scope, http, interval, format, messageBroker, location) => {
  ...
  scope.refreshStats = () => http.post(`/_db/_system/_api/cursor`, {cache:false,query:stat,bindVars:{time:6}}).then(data => scope.cfg.stats = data.data.result[0]);
  scope.refreshStats();
